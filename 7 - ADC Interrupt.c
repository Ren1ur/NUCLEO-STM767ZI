...
{   // Инициализация таймера TIM4 для запуска АЦП
    ...
    TIM4->PSC = 719;    // <- Менять здесь! 
    TIM4->ARR = 9999;   // <- !!! Тут не менять !!! Это самое адекватное значение.
    TIM4->ARR |= 0xF0000;
    TIM4->CR1 |= TIM_CR1_CEN;               // Запуск счета таймера
    /* 
        TIM4 Настроен таким образом, чтобы АЦП выполнял замеры 10 раз в 1 секунду
    */
}
