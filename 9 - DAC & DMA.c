...
void DAC_Init(void)
{   // Инициализация ЦАП
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN;            // Тактирование портов D
...
{   // Инициализация DMA
    ...
    DMA1_Stream5->CR |= (0x7U << DMA_SxCR_CHSEL_Pos);   // Выбор 7-го канала
    DMA1_Stream5->CR |= (0x1U << DMA_SxCR_MSIZE_Pos);   // Размер данных памяти 16 бит (полуслово)
    DMA1_Stream5->CR |= (0x1U << DMA_SxCR_PSIZE_Pos);   // Размер данных периферии 16 бит (полуслово)
    DMA1_Stream5->CR |= DMA_SxCR_CIRC;                  // Циклическая передача
    DMA1_Stream5->CR |= (0x1U << DMA_SxCR_DIR_Pos);         // Направление передачи (из памяти в периферию)

    DMA1_Stream5->NDTR = 72;                            // Количество данных (?)
    DMA1_Stream5->PAR |= (uint32_t) &DAC->DHR12R1;      // Запись адреса периферии
    DMA1_Stream5->M0AR |= (uint32_t) &sinus;            // Запись адреса памяти
    
    DMA1_Stream5->CR |= DMA_SxCR_EN;      // Включение DMA
}
...
{   // Инициализация таймера TIM4 для запуска АЦП
    ...
    //TIM4->PSC = 7199;    // - 1Hz
    TIM4->ARR = 99;   //  По умолчанию должно быть 9999 <- !!! Тут не менять !!! Это самое адекватное значение.
    TIM4->ARR |= 0xF0000;
